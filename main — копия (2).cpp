    ...
    for(k=3;k<12;k=k+2){ ///увеличиваем площадь квадрата
        for(i=imax-u;i<k+imax-d;i++)
            for ( j = jmax-l; j < k+jmax-r; j++) {
                if (buf[i][j]!=0)
                    if(result2[buf[i][j]-1]!=1&&result[buf[i][j]-1]!=0&&(result2[buf[i+1][j]-1]!=0||result2[buf[i-1][j]-1]!=0||result2[buf[i][j-1]-1]!=0||result2[buf[i][j+1]-1]!=0||result2[buf[i-1][j+1]-1]!=0||result2[buf[i+1][j-1]-1]!=0||result2[buf[i-1][j-1]-1]!=0)){///ищем пиксели
                    result2[buf[i][j]-1] = 1;
                    m+=1;
                    curr+=1;
                    s+=result[buf[i][j]-1];
